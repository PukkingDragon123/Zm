...
Z.shop = (function () {
  ...
  const LINES = ['Kitsune: "Every charm here is genuine. Mostly. The rune stones bite, so ask first."', 'Kitsune: "Coin or scrap, little tanuki. Nine tails, zero favors."', 'Kitsune: "That one came off a puppet that lost. It has something to prove."'];
  ...
  function sell(id) {
    const it = D.itemById(id); if (!it || Z.state.availableCount(id) < 1) { Z.audio.sfx.error(); Z.ui.toast("That's on your puppet", 'warn'); return; }
  ...
  function filterBar() {
    ...
    [['all', 'ALL'], ['chassis', 'FRAME'], ['generator', 'RUNE'], ['motor', 'WHEEL'], ['wheels', 'FEET'], ['weapon', 'WPN'], ['armor', 'ARM'], ['utility', 'CHARM']].forEach(([k, l]) => { const b = U.el('button', 'fbtn' + (filter === k ? ' on' : ''), l); b.addEventListener('click', () => { filter = k; render(); }); bar.appendChild(b); });
  }
